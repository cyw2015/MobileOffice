package com.cyw.mobileoffice.util;

/**
 * Created by cyw on 2016/5/27.
 */
public class AppURL {
    public final static String BASE = "http://cyw.ngrok.cc";
    public final static String LOGINURL = "http://cyw.ngrok.cc/login";
    public final static String GETEMPIMAGEURL = "http://cyw.ngrok.cc/person/getUserEmpImage.do";
    public final static String GETCONTACTS ="http://cyw.ngrok.cc/person/getContacts.do";/*通讯录*/
    public final static String GETVIEWDOCPAGE = "http://cyw.ngrok.cc/document/getViewDocPage.do";
    public static final String  IMAGEPATH ="http://cyw.ngrok.cc/imageEmp/" ;
}
