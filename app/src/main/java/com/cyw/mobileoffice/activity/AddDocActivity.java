...
public class AddDocActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void getRec(View v) {
        AlertDialog.Builder builder = new AlertDialog.Builder(AddDocActivity.this);
...
//                toast.show();
    ...
    public void addDoc(){
        ...
        if (docCode.isEmpty()){
            Toast.makeText(AddDocActivity.this,"公文编号不能为空",Toast.LENGTH_SHORT).show();
            return;
        }
        String  docTitle = et_doc_title.getText().toString();
        if(docTitle.isEmpty()){
            Toast.makeText(AddDocActivity.this,"公文标题不能为空",Toast.LENGTH_SHORT).show();
            return;
        }
        if(recipients.isEmpty()){
            Toast.makeText(AddDocActivity.this,"收文人不能为空",Toast.LENGTH_SHORT).show();
            return;
        }
        String docContent = et_doc_content.getText().toString();
        if(docContent.isEmpty()){
            Toast.makeText(AddDocActivity.this,"公文内容不能为空",Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        myDialog.setCancelable(false);
        myDialog.show();
        RequestParams params = new RequestParams(AppURL.ADDPUBDOC);
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (error == 0) {
                        Toast.makeText(AddDocActivity.this, "添加成功", Toast.LENGTH_SHORT).show();
                        myDialog.cancel();
                        finish();
                    } else {
                        Toast.makeText(AddDocActivity.this, msg.getErrorMsg(), Toast.LENGTH_SHORT).show();
                        myDialog.cancel();
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    Toast.makeText(AddDocActivity.this, "添加出错", Toast.LENGTH_SHORT).show();
                    myDialog.cancel();
                }
            }
            ...
            @Override
            public void onCancelled(CancelledException cex) {
                Toast.makeText(AddDocActivity.this, "添加取消", Toast.LENGTH_SHORT).show();
                myDialog.cancel();
            }
